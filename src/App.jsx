...
function IntroCutscene({ onDone }) {
  ...
  useEffect(() => {
    ...
    const loop = (now) => {
      ...
      const eventStart = 4.1;
      const blackout =
        t < eventStart ? 0 : t < 7.6 ? 1 : clamp((8 - t) / 0.6, 0, 1);
      const flash = clamp((t - eventStart) / 0.1, 0, 1) * (1 - blackout);
      ...
      // Sun (pixel square) before the fall
      if (t < 4.4) {
        const sunAlpha = clamp((eventStart - t) / 0.25, 0, 1);
        ctx.save();
        ctx.globalAlpha = 0.9 * sunAlpha;
        pixelRect(W * 0.48, H * 0.18, 28, 28, "#ffd28a");
        pixelRect(W * 0.485, H * 0.185, 18, 18, "#ffe6b3");
        ctx.restore();
      }
      ...
      if (t > 4.2 && t < 8)
        drawText("THE VEIL SWALLOWED THE SKY", H / 3, "#f2b97d");
      if (t > 8.2 && t < 11)
